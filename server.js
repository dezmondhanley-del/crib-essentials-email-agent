...
const leadSchema = new mongoose.Schema(
  {
    ...
    sentAt: Date,
    sentBody: String,
    // Extra messages sent after the reply ("your label is printed", etc.).
    followUps: [Object],
...
function formatCustomer(cust) {
  ...
  const lines = cust.orders.map((o) => {
    ...
    const parts = [
      `- ${o.name}, placed ${String(o.createdAt).slice(0, 10)}${age !== null ? ` (${age} days ago)` : ''} â€” ${o.financial}, ${o.fulfillment}, $${o.total}`,
...
app.post('/api/leads/:id/send', async (req, res) => {
  ...
  try {
    ...
    // A second message on an already-answered conversation is a follow-up:
    // the original reply stays as it was and the new one is added after it.
    const isFollowUp = lead.status === 'sent';
    if (dbReady && Lead) {
      if (isFollowUp) {
        await Lead.findByIdAndUpdate(req.params.id, { $push: { followUps: { sentAt, body: bodyText, attachments: stored } } });
      } else {
        await Lead.findByIdAndUpdate(req.params.id, { status: 'sent', sentAt, sentBody: bodyText, sentAttachments: stored });
      }
    } else {
      const m = memoryLeads.find((l) => l._id === req.params.id);
      if (m && isFollowUp) {
        m.followUps = (m.followUps || []).concat([{ sentAt, body: bodyText, attachments: stored }]);
      } else if (m) {
        m.status = 'sent';
        m.sentAt = sentAt;
        m.sentBody = bodyText;
    ...
    res.json({ sent: true, sentAt, attachments: stored.length, reminders, followUp: isFollowUp });
  } catch (err) {
...
app.get('/api/health', async (req, res) => {
  ...
  res.json({
    status: 'ok',
    version: 'v15.4 - follow-up messages on answered conversations; cost of goods on orders',
